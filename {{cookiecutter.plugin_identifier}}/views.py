...
"""
    ...
    ~~~~~~~~~~~~~~~~{{ '~' * cookiecutter.plugin_identifier|count+6 }}
...
{{ cookiecutter.plugin_identifier }} = Blueprint("{{ cookiecutter.plugin_identifier }}", __name__, template_folder="templates")
...
@{{ cookiecutter.plugin_identifier }}.route("/")
def index():
    plugin_obj = get_plugin_from_all({{ cookiecutter.plugin_identifier }})
    return render_template("index.html", plugin_obj=plugin_obj)
